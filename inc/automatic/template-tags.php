...
if ( ! function_exists( 'doctorpedia_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 */
	function doctorpedia_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'doctorpedia' ),
...
if ( ! function_exists( 'doctorpedia_posted_by' ) ) :
	/**
	 * Prints HTML with meta information for the current author.
	 */
	function doctorpedia_posted_by() {
		$byline = sprintf(
			/* translators: %s: post author. */
			esc_html_x( 'by %s', 'post author', 'doctorpedia' ),
...
if ( ! function_exists( 'doctorpedia_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function doctorpedia_entry_footer() {
		// Hide category and tag text for pages.
		if ( 'post' === get_post_type() ) {
			/* translators: used between list items, there is a space after the comma */
			$categories_list = get_the_category_list( esc_html__( ', ', 'doctorpedia' ) );
			if ( $categories_list ) {
				/* translators: 1: list of categories. */
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'doctorpedia' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'doctorpedia' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'doctorpedia' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'doctorpedia' ),
						array(
							'span' => array(
								'class' => array(),
							),
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'doctorpedia' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'doctorpedia_post_thumbnail' ) ) :
	/**
	 ...
	 */
	function doctorpedia_post_thumbnail() {
...
if( !function_exists( 'doctorpedia_heading' ) ){
	 ...
	 */
	function doctorpedia_heading( $field_name = 'heading', $params = [] ){
		$params += [
...
if( !function_exists( 'doctorpedia_button_group' ) ){

	/**
	 * Output standard button group support group contents
	 * Can be used for sub fields or top level by passing the appropriate function param
	 */
	function doctorpedia_button_group( $field_name = 'button_section', $params = [] ){
